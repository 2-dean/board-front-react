...
Fn.PluginVers.ibcommon = {
  name: 'ibcommon',
  version: '1.0.17-20250213-13'
};

/* 20230406 배차일보 속도가 느려 아이비시트 김상현 엔지니어분 도움을 받아 추가 */ 
Fn.findText2 = function(colName, str){
  var rtnRow;
  this.getDataRows().every(r=>{
    if(r[colName] === str){
      rtnRow = r;
      return false;
    }
...
_IBSheet.CommonOptions = {
  Cfg: {
    Export: {
      Url: "/ibsheet8/sheet/jsp/Down2Excel.jsp"
    }, // 엑셀다운 URL
    Alternate: 0, // 홀짝 행에 대한 배경색 설정
    InfoRowConfig: {
      Visible: 1,
      Layout: ["Count"],
      Space: "Top"
    ...
    SuppressMessage: 4, //시트메시지표시
	SuppressExportMessage: 1,
    ...
    RequiredPosition: "Right", // 헤더의 오른쪽에 Required를 설정    20230105 진성욱
    ColorState: "8",	//입력수정삭제상태 ROW 색상표시삭제
    UseHeaderSortCancel: true, //헤더소팅취소 추가
    NoTreeLines: true, //트리노드연결선 삭제
    SearchProgress: true, //검색프로그레스
    HeaderCheckMode: 1, //보이는행만 체크
    StandardFilter: 3, //트리검색 시, 자식행까지 검색(비공개 기능)
    MsgLocale: "Ko", // 언어 메시지 파일 기본 설정 값
    PasteStrictMode: 1, // 숫자 컬럼에 Ctrl + v 붙여넣기 시 엄격하게 체크하여 붙여넣기
  },
  Def: {
	InfoRow: {
    	Height: 24 //20230112 진성욱
    },
    Header: { //헤더 영역 행에 대한 설정
      ShowHint : 0, //헤더는 힌트 기능 사용하지 않음
      Menu: {
        Items: [
		  /*{
            "Name": "컬럼 감추기"
  ...
  Events: {
    ...
    "onRenderFirstFinish": function (evtParam) {
		//2번호출되서 조건넣음
		if ('IBSHEET' == evtParam.sheet.id.substring(0, 7)){
			//개인화 DB저장 내용 불러옴 20240106	진성욱
			xcommon.getSheetInfo(evtParam.sheet);
		} 	
    },  
    "onSelectFile": function (evtParam) { //엑셀 업로드후 그리드 ENUM정보가 사라지는 문제가 발생하여 추가 20240528
		xgrid.doSetGridEtcData(evtParam.sheet.etc);
    },   
	"onReceiveData": function (evtParam){
		if(evtParam.type == 'EXCEL') {
			var DATA = evtParam.data;
			if (typeof DATA == 'string') DATA = JSON.parse(DATA);

			for (var row of DATA) {
				var cols = evtParam.sheet.getCols({seq: 1});
				for (var col of cols) {
					var size = evtParam.sheet.getAttribute(null, col, 'Size');
					if (size) {
						row[col] = row[col].substr(0, size);
					}
				}
			}
		}
...
window.IB_Preset = {
  ...
  "STATUS": {
    ...
    Format: {
      'I': "<img src='/ibsheet8//img/ic_insert.png'/>",
      'U': "<img src='/ibsheet8/img/ic_update.png'>",
      'D': "<img src='/ibsheet8/img/ic_delete.png'>",
      'R': ''
    }
  },
  // DelCheck Type
