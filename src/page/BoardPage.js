...
const BoardPage = () => {
    ...
    console.log("[ BoardPage ] 0. 로그인 사용자 정보확인 : \n\n " +
                "loginUser ID: " + loginUser.id +  ", isLogin: " + loginUser.isLogin);
    console.log("[boardPage] 0. 페이지번호 : " + activePage);



    console.log("[ BoardPage ] 1. BoardApi 요청");
    console.log("[localStorage 토큰확인]" + localStorage.getItem('token'))
    Api.get("/boards")
        .then((response) => {
